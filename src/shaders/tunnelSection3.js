/** Section 3 tunnel — violet-tinted twist on the base tunnel. */

export const tunnelSection3VertexShader = `void main(){ gl_Position = vec4(position, 1.0); }`;

export const tunnelSection3FragmentShader = `
uniform float iTime;
...
#define POINT_SIZE 1.9
#define POINT_COLOR_A vec3(0.92, 0.94, 1.0)
#define POINT_COLOR_B vec3(0.55, 0.58, 0.95)
#define SPEED 0.75
...
vec2 TunnelPath(float x){
  vec2 offs = vec2(
    0.25 * sin(TAU * x * 0.45) + 0.35 * sin(TAU * x * 0.18 + 0.5),
    0.28 * cos(TAU * x * 0.28) + 0.22 * cos(TAU * x * 0.12)
  );
